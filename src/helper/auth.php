...
class Auth {
	...
	static public function load_capabilities() {
		...
		$db = DB::instance();
		$capabilities = $db->get_result('SELECT customer_entity_id, capability FROM person_capabilities WHERE person_entity_id = ?', self::get_id());
		
		$_SESSION['user']['caps'] = array();
		
		foreach($capabilities as $cap) {
			$customer_id = (int)$cap['customer_entity_id'];
